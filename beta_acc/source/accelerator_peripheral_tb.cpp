...
//     output = accelerator_peripheral(w2, bias_2, 1, rx_stream, tx_stream, initialized_flag,
//     self_test, epoch, complete, test);

//     //initialized = true;
//     prediciton = accelerator_peripheral(output.new_w2, output.new_b2, 0, rx_stream,
//     tx_stream, initialized_flag, self_test, epoch, complete, test);
...
int main() {
    ...
    output = accelerator_peripheral(w2, bias_2, training, rx_stream, rx_stream, 
        initialized, self_test, epoch, complete, test);


    // std::thread beta_thread(foo, std::ref(w2), std::ref(bias_2),
    //  std::ref(training), std::ref(rx_stream), std::ref(tx_stream),
