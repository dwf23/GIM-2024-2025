...
typedef struct {
    ...
    XPulse_gen_uio_map maps[ MAX_UIO_MAPS ];
} XPulse_gen_uio_info;

/***************** Variable Definitions **************************************/
static XPulse_gen_uio_info uio_info;
...
static int uio_info_read_name(XPulse_gen_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XPulse_gen_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XPulse_gen_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XPulse_gen_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XPulse_gen_Initialize(XPulse_gen *InstancePtr, const char* InstanceName) {
	XPulse_gen_uio_info *InfoPtr = &uio_info;
