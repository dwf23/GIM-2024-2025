...
int &debug) {
    ...
    */
    ...
    ap_uint<16*2> bitstream;
    bool bit;
    const int bits_arr = 16*2;
    pkt input_packet;
    int input_packet_id;
    fixed_16 input_packet_data[2];
    debug = 0;
    ...
    while(flag){
        if(data_out.read_nb(input_packet)){
            ...
            input_packet_data[0] = input_packet.data[0];
            input_packet_data[1] = input_packet.data[1];
