// This is the test bench file for the GIM@Rice sub accelerator in HLS
#include "gim_model.h"
using namespace std;
...
int main() {
    ...
    // training the array
    Inference training = sub_accelerator(w2, bias_2, 1);

    // running inference using the trained accelerator
    Inference output = sub_accelerator(training.new_w2, training.new_b2, 0);

    // capture the outputs of the accelerator
    cout << "Sub done with training!" << endl;
