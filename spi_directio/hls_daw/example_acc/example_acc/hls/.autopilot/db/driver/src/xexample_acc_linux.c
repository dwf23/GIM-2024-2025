...
typedef struct {
    ...
    XExample_acc_uio_map maps[ MAX_UIO_MAPS ];
} XExample_acc_uio_info;

/***************** Variable Definitions **************************************/
static XExample_acc_uio_info uio_info;
...
static int uio_info_read_name(XExample_acc_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XExample_acc_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XExample_acc_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XExample_acc_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XExample_acc_Initialize(XExample_acc *InstancePtr, const char* InstanceName) {
	XExample_acc_uio_info *InfoPtr = &uio_info;
